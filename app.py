...
def test_connection():
    with app.app_context():
        ...
        def login():
            if request.method=='POST':
                ...
                if user and password==user.password:
                    ...
                    flash(f'Welcome to back PromoDesk {email}', 'success')
                    return redirect('/')
                else:
        ...
        def investorpage():
            if request.method=="POST":
                ...
                if user.password==password:
                    investor=Investor(email=email,category=category)
        ...
        def investorready():
            ...
            return render_template('investorsready.html')
        ...
        def mentor():
            if request.method=='POST':
                ...
                Mentor=MENTOR(email=email,password=password,name=firstname,surname=surname,experience=experience,category=category)
        ...
        def mentorready():
            if request.method=='POST':
                ...
                if user.password==passwordAttempt:
                    print(email)
                    businessesss =business.query.filter_by(email=email).first()
            ...
            return render_template("mentorready.html")
